...
class Random_vFlip(object):
    """Flip image about vertical axis Randomly."""        

    def __call__(self, sample):
        image, key_pts = sample['image'], sample['keypoints']
        
        image_ = np.copy(image)
        key_pts_ = np.copy(key_pts)
        
        to_flip = np.random.randint(2, size=1)
        if(to_flip[0] == 1):
            image_ = np.fliplr(image_)
            key_pts_[:, 0], key_pts_[:, 1] = image_.shape[1] - key_pts_[:, 0], key_pts_[:, 1]
        
        image_ = image_.copy()
        
        return {'image': image_, 'keypoints': key_pts_}
